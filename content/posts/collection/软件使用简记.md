...
- [Keyviz](https://github.com/mulaRahul/keyviz)
    ...
    最然不知道有什么用但总会用上的XD。
- [x] [Pot (Translator of Pylogmon)](https://github.com/pot-app/pot-desktop)
...
- [x] [catime](https://github.com/vladelaina/Catime/blob/main/i18n/README_zh-CN.md)
    很轻量的时钟，透明背景可以放置也方便。我用来当下班倒计时w

## 待定

- [ ] [proxypin](https://github.com/wanghongenpin/proxypin)
    抓包工具。以前试过好几个都不算好用。而且抓包工具很多都是商业闭源软件。
    这个是开源的，可以试试。
- [Netcatty](https://github.com/binaricat/Netcatty)
    现代化的 SSH 客户端、SFTP 浏览器与终端管理器，内置 AI 智能体。原生支持 macOS、Windows 与 Linux，完全开源。
    一致用的windows自带的openssh，试试专业的ssh客户端。
- [tmux](https://github.com/tmux/tmux)
    tmux 是一个终端复用器：它允许从单个屏幕创建、访问和控制多个终端。tmux 可以从屏幕中分离出来并在后台继续运行，然后稍后重新连接。
    试了一下，功能还是好用的，就是用法有点晦涩，不太习惯。
- [wireguard](https://www.wireguard.com/)
    VPN工具，可以很方便地实现流量加密、异地组网和内网穿透等。因为ssh和ruskdesk被阻断用不了所以配了这个，配置还算简单使用体验也还不错。
- [dbx](https://github.com/t8y2/dbx)
    数据库连接管理工具。rust+Tauri技术栈，体积很小只有15M，但内存占用并不小，也要一两百M。不过比现在用的Java技术栈的DBGate的400+M要好，而且功能也全。
    现在应该是两者并行使用适应新工具的操作，迁移不迁移再说。
    用了一段时间还不错。最主要是没有直接提供表备份功能，此外表结构编辑和字段查询不方便，其他日用基本已经习惯dbx了。
- [ShareX](https://github.com/ShareX/ShareX)
    开源截屏录屏工具。每次用qq/wx的截屏还要打开应用，找个专门的工具用好了。
    初步体验还行，界面有点丑，但反正各种乱七八糟的功能用不上也无所谓。就是需要重新培养截图快捷键习惯。
...
- [ ] [FileConverter](https://github.com/Tichau/FileConverter)
    ...
    卸载了万恶的wps，但也因此缺失了一些辅助的小功能，比如文件格式转换。
    就是文件转换需求过少经常想不起来用就是了。
- [x] [Koodo Reader](https://www.koodoreader.com/zh)
    还不错的阅读器，只是我现在都是用更精简的SumatraPDF。
...
- [x] [Localtunnel](https://theboroer.github.io/localtunnel-www/)
...
- [x] [InputTip](https://inputtip.pages.dev/)
...
- [x] [pakeplus](https://www.pakeplus.com/)
    网页打包为应用，win，Android和ios都支持，而且打包的体积很小。
    作者盗用token，爬。
- [ ] [cherry studio](https://cherry-ai.com/)
    比较常见的LLM客户端，不过集成的供应商够多，功能也完善，在社区中算是小有名气，就用的这个。
    后期更新越来越臃肿也越来越卡。反正我也用不上什么高级功能，基本以对话为主，干脆用网页还快捷点。
- [x] [沉浸式翻译](https://immersivetranslate.com/zh-Hans/)
    确实是很好用的翻译插件，`alt+A`全页翻译，翻译就在原文旁边，保持格式一致不会弹窗什么的很突兀。搭个阅读插件使用效果更佳，都能用来专门学外语了。
    商业化后虽然没什么大问题但也是节奏不断多少有点膈应，弃了换开源替代了。
- [x] [MarkFlowy](https://github.com/drl990114/MarkFlowy)
...
- [x] [treex](https://github.com/shiquda/treex/blob/main/docs/README_zh-cn.md)
    ...
    没咋用过。
- [x] [optimizer](https://github.com/hellzerg/optimizer)
...
- [ ] [MrRSS](https://github.com/WCY-dt/MrRSS)
    ...
    弃用folo，试试这个rss阅读器。但并不算好用，最大的问题是已读状态保存不了，每次重启都全未读。等了几个月都没修直接弃了。
