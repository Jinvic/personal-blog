...
## Hello Github 月刊【98-125】

### 复古游戏模拟器 RetroArch |cmp
...
### 极简的 Android 语音录音应用 Voice-Recorder |save
...
### 宝可梦存档编辑工具 PKHeX |save

宝可梦存档编辑工具。这是一款开源的宝可梦存档编辑器，支持多种宝可梦游戏存档文件的读取与编辑。用户可自由修改宝可梦属性、技能、道具、图鉴完成度等信息，并内置存档合法性验证功能，确保修改后的存档安全可用。
看起来还不错，以后入坑可以用上|115

### 图形编辑工具 PixiEditor |save
...
|113
体验一般，就一普通的记账软件，没啥特别的。

### rsync图形界面 RsyncUI |save
...
ps3模拟器，时代的眼泪，泪目~
记得很久以前试过，并不成熟+电脑差啥也跑不起来，不过名字还是记下了。
ppsspp,pcsx2,rpcs3,dolphin,cemu,xemu,desmume...
怀念以前的用模拟器玩游戏的时光，如今pc上的游戏都玩不过来，也没空去用模拟器补老游戏或其他平台的游戏了。 |92
...
### 本地服务器管理面板 LeePanel |todo

安装在本地计算机的远程服务器管理面板，所有操作都由 SSH 操作完成，不需要在服务器安装任何东西。
如果只有一台电脑应该挺好用，否则还是各服务器自己装比较合适。只是转移了复杂度。 |408

### 图像处理软件 pinta |cmp

[pinta](https://www.pinta-project.com/)
PhotoShop开源替代，但好不好用存疑 |396

### 本地服务暴露公网 Tunelo |todo

[Tunelo](https://github.com/jiweiyuan/tunelo)
一行命令将本地服务暴露到公网，只需要单个 4MB 二进制文件，使用 QUIC 协议。 |390
类似Localtunnel的内网穿透小工具。frp虽然强大，但配置起来麻烦。这类小工具在临时使用时更方便。

### 桌面端 + 安卓端的电子书阅读工具 ReadAny |save

[ReadAny](https://github.com/codedogQBY/ReadAny)
桌面端 + 安卓端的电子书阅读工具，自带 AI 功能、语音朗读和多端同步。 |390
存。一个普通的阅读器项目。

### mdto.page |save

[mdto.page](https://github.com/hjinco/mdto)
这个网站免费将 Markdown 文件转成 HTML 格式，发布成公开访问的网页。 |382
以后临时分享md可能用到。

### 极简风格的图床 云图 |save

[云图](https://github.com/qazzxxx/cloudimgs)
一个极简风格的图床，可以搭建到自己的 NAS，提供灵活的 API。 |380
图床+1

### 任务管理应用 Tada |todo

[Tada](https://github.com/LoadShine/tada)
开源的任务管理应用，带有 AI 总结功能。 |379

### git仓库996程度分析 code996 |done

一个命令行工具，通过分析 Git 仓库的提交时间，判断项目的编码工作强度和加班情况。
本来想看乐子，试一下发现自己996指数奇高，乐不出来了。 |376

### 可视化生成图表 oxdraw |todo

[oxdraw](https://github.com/RohanAdwankar/oxdraw)
一个 Rust 语言编写的 Web 服务，用来可视化生成图表，同时给出对应的 Mermaid 语法代码。
可视化画图+生成mermaid代码。感觉挺方便的，不用学mermaid了。 |371

### 网页抓取查看工具 Ladder |save
...
### 开源去中心化点对点网络 kulfi |cmp

[kulfi](https://github.com/fastn-stack/kulfi)
点对点端口共享，类似于frp |365
...
比起装一个专门的软件，网页或许会更方便。|363

### 远程桌面工具 P2P Remote Desktop |cmp
...
### RSS阅读器 Glean 拾灵 |save

[Glean 拾灵](https://github.com/LeslieLeung/glean)
周刊作者做的项目，值得关注。 | 87
...
和[skills-manager](https://github.com/xingkongliang/skills-manager)类似的项目。|275

### 服务器运维管理面板 panel |cmp

[panel](https://github.com/acepanel/panel)
项目说明：一个基于 Go 语言开发的服务器运维管理面板，能够提供高效运维和离线运行，旨在简化企业级服务器管理，支持主流 Linux 系统使用。
如果够轻量的话还是有竞争力的，就是没试过 |269

### AI 编码代理 DeepSeek-Reasonix | todo
...
基于yt-dlp的话应该能兼容我的现有工作流，就是总感觉好像用过（ |268

### 桌面媒体下载器 | cmp

[omniget](https://github.com/tonhowtf/omniget)
一个基于 Tauri 和 Rust 开发的桌面媒体下载器，能够从 YouTube、Instagram、TikTok、Twitter 等 12 个平台原生下载视频和媒体文件，并通过 yt-dlp 支持 1000+额外站点。旨在提供轻量高效、无浏览器扩展依赖的下载体验，支持 Udemy 和 Hotmart 课程批量下载、Telegram 媒体获取、格式转换等功能，跨平台支持 Windows、macOS 和 Linux 系统使用。
存 |261

### 开源 RSS 阅读器 | done

[MrRSS](https://github.com/WCY-dt/MrRSS)
项目说明：一个基于 Wails v3 构建的现代化开源 RSS 阅读器，集成了 AI 自动摘要、全文抓取及多引擎翻译功能，支持 Windows、macOS 和 Linux 平台，旨在为用户提供私密、高效且智能的本地阅读体验。|252
folo现在拉了，换一个rss阅读器用。

### 跨平台小说阅读器 IReader

[IReader](https://github.com/IReaderorg/IReader)
项目说明：一款跨平台小说阅读器，通过扩展和 JavaScript 插件支持多种书源。支持Android 和桌面端。| 248
小说阅读器+1，支持导入书源。

### 重写 Git 仓库历史记录 filter-repo-rs |save

[filter-repo-rs](https://github.com/Cactusinhand/filter-repo-rs) |243
项目说明：用于高效重写 Git 仓库历史记录。使用流水线架构，可以删除敏感数据、缩小仓库规模并重构项目。

### 全文搜索引擎 blaze |save

[blaze](https://github.com/wizenheimer/blaze)
项目说明：一个轻量的 Go 语言实现的全文搜索引擎，可用在博客等场景。 |242

### 跨平台下载器 pikatorrent |save

[pikatorrent](https://github.com/G-Ray/pikatorrent)
项目说明：一款新一代的 Torrent 客户端，它是一款美观、轻量且跨平台的应用程序，可安装在您的所有设备上。
存，但很少用得上 |239

### 自动化同步工具 taosync|save

[taosync](https://github.com/dr34m-cn/taosync)
项目说明：一个适用于OpenList v3+的自动化同步工具。
功能还是很有用的，就是这个名字让人无力吐槽 |237

### 本地代码转换为promptyour-source-to-prompt.html |todo

[your-source-to-prompt.html](https://github.com/Dicklesworthstone/your-source-to-prompt.html)
...
我现在用的vfox+just。有空可以试试这个。 |268

### Surge：Go 写的极速开源 TUI 下载管理器 |cmp
...
字幕存。Aegisub有些复杂了，不知道这个会不会好些|240
...
## Self-Host Weekly【2026-07-24 - 2026-01-16】

### 轻量级、自托管的笔记应用 NoteDiscovery |todo

[NoteDiscovery](https://github.com/gamosoft/NoteDiscovery)
NoteDiscovery 是一款轻量级、自托管的笔记应用，让您完全掌控您的知识库。用美观、现代的界面撰写、组织、发现您的笔记——所有这些都在您自己的服务器上运行。
Obsidian开源替代，可以考虑使用这个。

### 云存储/分享/协作平台 Sync-in |cmp

[Sync-in](https://github.com/Sync-in/server)
Sync-in是一个云存储/分享/协作平台，它之所以被列入其中，是因为它试图取代被认为臃肿的Nextcloud，许多社区用户对此抱怨。它可能没有它的竞争对手那样美观，但它对资源更友好，除了需要一个MariaDB数据库外，不需要外部依赖。
云盘项目，openlist更多是一个聚合作用，作为云盘本身的功能比较孱弱。以后有自建网盘需求可以考虑。

### 免费书籍下载平台 Shelfmark

[Shelfmark](https://github.com/calibrain/shelfmark)
Calibre-Web 自动下载器，一个用于下载免费（并且绝对不是盗版）书籍的平台，已经更名为 Shelfmark。
存。
...
无代码工具，数据库可视化展示和操作，Collectify上位替代。

### Go的GUI库 cogentcore

[cogentcore](https://github.com/cogentcore/core)
GoGui+1

### 终端显示GUI term.everything

[term.everything](https://github.com/mmulet/term.everything)
在终端中显示gui程序的画面，有点意思。

### Nginx 日志分析可视化 NginxPulse |todo

[NginxPulse](https://github.com/likaia/nginxpulse/)
轻量级 Nginx 访问日志分析与可视化面板，提供实时统计、PV 过滤、IP 归属地与客户端解析。
大概类似非侵入的流量分析，直接分析nginx日志，感觉还不错。

### 一键本地生SSL证书​ lcl.host |save

试用了一下感觉还行，证书会和本地项目关联。提供了平台：anchor.dev查看，同时也有手把手的证书配置教程。
